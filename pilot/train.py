...
def train_grpo_llama():
  ...
  bnb_config = BitsAndBytesConfig(load_in_4bit=True, llm_int8_enable_fp32_cpu_offload=True)  # Enables CPU offloading for non-GPU machines

  model = LlamaForCausalLM.from_pretrained("huggyllama/llama-7b", quantization_config=bnb_config, device_map=device)
  ...
  reference_model = GRPONetwork(LlamaForCausalLM.from_pretrained("huggyllama/llama-7b", quantization_config=bnb_config, device_map=device)).eval()

  ref_sd = get_peft_model_state_dict(policy_model)  # Correct way to extract state_dict for PEFT models
  ...
  for epoch in range(epochs):
    reference_model.load_state_dict(ref_sd, strict=False)  # Allow missing keys to avoid quantization mismatch
    group_returns = []
    input_batches = []
    ...
    group_buffer.add(get_peft_model_state_dict(policy_model), group_avg)  # Save correct PEFT state_dict

    if group_avg > 0:
      ref_sd = get_peft_model_state_dict(policy_model)  # Save correct reference state_dict
