...
def merge_json_files(input_dir=".predictions", output_file="merged_predictions.json"):
  ...
  print(f"Found {len(json_files)} JSON files to merge: {json_files}")
    ...
    try:
      with open(json_file, "r", encoding="utf-8") as f:
        ...
        if not isinstance(data, dict):
          print(f"File {json_file} does not contain a dictionary: {type(data)}")
          continue
        merged_data.update(data)  # Later files overwrite earlier ones
        print(f"Processed {json_file} with {len(data)} entries")
    except json.JSONDecodeError as e:
      print(f"Failed to decode JSON in {json_file}: {e}")
  ...
  try:
    ...
    print(f"Merged data saved to {output_file} with {len(merged_data)} entries")
  except Exception as e:
    print(f"Failed to write merged data to {output_file}: {e}")
